...
@ApplyOptions<Command.Options>({
	name: "softban",
	description: "Softbans a server member from the server",
	usage: "<user> [reason]",
	...
	chatInputCommand: {
		...
		options: [
			...
			{
				name: "reason",
				description: "The reason why you softbanned them from the server.",
				type: "STRING",
				required: false
			}
		]
...
export default class extends Command {
	public async messageRun(message: Message<true>, args: Command.Args): Promise<void> {
		...
		await message.reply(this.t(locale, "mod_commands:softban.response", { user: member.user.tag, reason: reason ?? "-" }));
	}
	...
	public async chatInputRun(interaction: CommandInteraction<"cached">): Promise<void> {
		...
		await interaction.followUp(this.t(interaction.locale, "mod_commands:softban.response", { user: member.user.tag, reason: reason ?? "-" }));
	}
	...
	private async sharedRun(user: User, moderator: User, locale: string, guildId: string, reason?: string) {
		...
		await modlog.create({
			...
			modlogType: ModlogType.SOFTBAN,
			reason,
			id
		});

		await this.client.modaction.softban(modlog);
	}
}
